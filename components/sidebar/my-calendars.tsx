  ...
  return (
    <Accordion type={"single" } collapsible>
      <AccordionItem value={"item-1"}>
        <AccordionTrigger className={"justify-normal gap-32 text-sm hover:no-underline text-white"}>
          My Calendars
        </AccordionTrigger>
          ...
          {myCalendars.map((cal) => (
            <div className = " items-top flex space-x-2" key={cal.id}>
              ...
              <div className="grid gap-1.5 leading-none">
                <label htmlFor={cal.id} className={"text-sm font-medium leading-none text-gray-600 peer-disabled:cursor-not-allowed peer-disabled:opacity-70"}>
                  {cal.title}
                </label>
              </div>
            </div>
